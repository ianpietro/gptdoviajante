...
import { AFFILIATE_CONFIG, BYPASS_LOGIN, FEATURES } from './config.js';
...
import { partnerConfig, buildAffiliateLink, evaluateTripOpportunities, analytics } from './modules/partnerEngine.js';
...
async function handleUserSendMessage() {
  ...
  try {
    ...
    if (response.status === 429) {
      ...
      if (errData.code === 'AI_LIMIT_EXCEEDED') window.showPaywall('ai_limit', errData.error);
      else appendMessageBubble('assistant', `⚠️ **Muitas mensagens:** ${errData.error || 'Aguarde um momento.'}`, null, 'plan');
      return;
    }
...
async function handleTravelSendMessage() {
  ...
  try {
    ...
    if (response.status === 429) {
      ...
      if (errData.code === 'AI_LIMIT_EXCEEDED') window.showPaywall('ai_limit', errData.error);
      else appendMessageBubble('assistant', `⚠️ **Muitas mensagens:** ${errData.error || 'Aguarde um momento.'}`, null, 'travel');
      return;
    }
    if (response.status === 401 || response.status === 403) {
      const errData = await response.json().catch(() => ({}));
...
function renderPackingChecklist() {
  ...
  const savedState = localStorage.getItem(getTripStorageKey("gptViajante_packingState"));
...
function saveChecklistState() {
  ...
  localStorage.setItem(getTripStorageKey("gptViajante_packingState"), JSON.stringify(checkedItems));
  updateChecklistProgress();
}
...
async function triggerAiLogisticsSync() {
  ...
  try {
    ...
    if (response.status === 429) {
      const errData = await response.json().catch(() => ({}));
      if (errData.code === 'AI_LIMIT_EXCEEDED') window.showPaywall('ai_limit', errData.error);
      throw new Error(errData.error || 'Muitas mensagens. Aguarde um momento.');
    }
    if (response.status === 401 || response.status === 403) {
      const errData = await response.json().catch(() => ({}));
